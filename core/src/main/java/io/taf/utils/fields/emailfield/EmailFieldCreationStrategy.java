...
public class EmailFieldCreationStrategy implements FieldCreationStrategy<EmailField> {

    @Getter(onMethod_ = {@Nonnull, @Override})
    private final Class<EmailFieldConfig> markedAnnotation = EmailFieldConfig.class;

    @Nonnull
    @Override
    public EmailField createField(@Nonnull Field field, @Nonnull Function<String, String> textResolver) {
        var annotation = field.getAnnotation(getMarkedAnnotation());

        var component = new EmailField();
